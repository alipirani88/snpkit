...
- core_snp_consensus: contain various combination of core/non-core consensus fasta files for individual samples and annotaed vcf results. 

- gubbins: contains various combination of core/non-core multi-fasta consensus generated by merging individual consensus files in core_snp_consensus folder. 
...
Note: non-core variant positions here refers to the positions where a variant was observed but was filtered out for not meeting one of below criteria. non-core variant positions therefore will contain a reference allele in this consensus.

- core variant criteria: not a core variant as it was filtered out in another sample
- hard variant filters: didn't meet hard variant filters such as DP(depth), MQ(Mapping quality), FQ(all reads supporting one variant), QUAL(variant quality), proximate_snp (the variant was in close proximity to another variant by 10 bp range)
- functional filters: fall in repetitive, masked or phage region

**date_time_referencegenome_allele_var_consensus.fa:** consensus generated from all unique variant positions that was called in any sample regardless of it being a core/non-core. This will contain all core variant positions, non-core variant positions substituted with dash for being unmapped and N’s for not meeting hard variant filters or falling under functional class filter. non-core variant that met all the filter parameters is substituted with actual variant allele.

**date_time_referencegenome_ref_allele_var_consensus.fa** full consensus relative to the reference genome generated from all unique variant positions that was called in any sample regardless of it being a core/non-core. This will contain all core variant positions, non-core variant positions substituted with dash for being unmapped and N’s for not meeting hard variant filters or falling under functional class filter. non-core variant that met all the filter parameters is substituted with actual variant allele.

**date_time_referencegenome_ref_allele_unmapped_consensus.fa:** full consensus relative to the reference genome generated from all unique variant positions that was called in any sample. This will contain all core variant positions, non-core variant positions substituted with dash for being unmapped and N’s for not meeting hard variant filters or falling under functional class filter. non-core variant that met all the filter parameters is substituted with actual variant allele. Positions that were unmapped relative to the reference genome are substituted with dashes.


- data_matrix: contains different types of data matrices and reports useful for variant diagnostics. 
...
SNP_matrix_code.csv and Indel_matrix_code.csv: contains one of the five status code for each unique variant positions(rownames) called for each sample(columns). 
