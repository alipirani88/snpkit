...
def extract_only_ref_variant_fasta_alternate():
    ...
    for i in xrange(1, end, 1):
        ...
        if sample_name_re == os.path.basename(args.filter2_only_snp_vcf_filename).replace('_filter2_final.vcf_no_proximate_snp.vcf', ''):
            ...
            count = 0
            for ntd in get_reference[ref_id[0]][:]:
                count = count + 1
                if count in unique_position_array:
                    pos_index = unique_position_array.index(count)
                    allele_var = str(variant_allele_array[0][pos_index])
                    ref_print_string = ref_print_string + allele_var + "\n"
                else:
                    allele_var = str(ntd)
                    ref_print_string = ref_print_string + allele_var + "\n"
            allele_ref_variant_fasta.write(ref_print_string)
            allele_ref_variant_fasta.close()
