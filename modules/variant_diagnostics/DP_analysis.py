...
def filtered_DP_analysis():
    print "Analyzing depth of all the filtered positions..."
    extract_DP_positions = "awk -F\'\\t\' \'{print $1}\' Only_filtered_positions_for_closely_matrix.txt | sed \'/^$/d\' > filtered_extract_DP_positions.txt"
    os.system(extract_DP_positions)
    ...
    filtered_DP_values_file = "%s/%s_filtered_DP_values" % (args.filter2_only_snp_vcf_dir, analysis)
    filtered_f2 = open(filtered_DP_values_file, 'w+')
    ...
    with open("%s/Only_filtered_positions_for_closely_matrix.txt" % args.filter2_only_snp_vcf_dir, 'rU') as csv_filess:
        csv_readerr = csv.reader(csv_filess, delimiter='\t')
        next(csv_readerr, None)
        for rows in csv_readerr:
            #print rows
