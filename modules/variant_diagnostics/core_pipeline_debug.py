...
def create_job(jobrun, vcf_filenames, unique_position_file, tmp_dir):
    ...
    if jobrun == "parallel-cluster":
        ...
        for i in vcf_filenames:
            ...
            args.filter2_only_snp_vcf_dir, i, unique_position_file, tmp_dir)
...
def prepare_snpEff_db(reference_basename):
    ...
    if os.path.isfile("%s/%s/snpEff.config" % (
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'])):
        ...
        keep_logging("cp %s/%s/snpEff.config %s" % (
        ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
        args.filter2_only_snp_vcf_dir), "cp %s/%s/snpEff.config %s" % (
                     ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
                     args.filter2_only_snp_vcf_dir), logger, 'debug')
        call("cp %s/%s/snpEff.config %s" % (
        ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
        args.filter2_only_snp_vcf_dir), logger)
    else:
        ...
        ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin']),
                     ...
                     logger, 'exception')
        exit()
    make_sure_path_exists("%s/%s/data/%s" % (
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    reference_basename[0]))
    make_sure_path_exists("%s/%s/data/genomes/" % (
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin']))
    ...
    keep_logging("cp %s %s/%s/data/genomes/%s.fa" % (
    args.reference, ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    reference_basename[0]), "cp %s %s/%s/data/genomes/" % (
                 args.reference, ConfigSectionMap("bin_path", Config)['binbase'],
                 ConfigSectionMap("snpeff", Config)['snpeff_bin']), logger, 'debug')
    call("cp %s %s/%s/data/genomes/%s.fa" % (
    args.reference, ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    reference_basename[0]), logger)
    ...
    keep_logging("java -jar %s/%s/%s build -genbank -v %s -c %s/snpEff.config -dataDir %s/%s/data" % (
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    ...
    call("java -jar %s/%s/%s build -genbank -v %s -c %s/snpEff.config -dataDir %s/%s/data" % (
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    ConfigSectionMap("snpeff", Config)['base_cmd'], reference_basename[0], args.filter2_only_snp_vcf_dir,
    ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin']), logger)
...
def variant_annotation():
    ...
    if ConfigSectionMap("snpeff", Config)['prebuild'] == "yes":
        if ConfigSectionMap("snpeff", Config)['db']:
            ...
            proc = subprocess.Popen(["java -jar %s/%s/%s databases | grep %s" % (
            ConfigSectionMap("bin_path", Config)['binbase'], ConfigSectionMap("snpeff", Config)['snpeff_bin'],
    ...
    for i in vcf_filenames:
        ...
        annotate_vcf_cmd = "java -Xmx4g -jar %s/%s/%s -csvStats %s_ANN.csv -dataDir %s/%s/data/ %s -c %s/snpEff.config %s %s > %s_ANN.vcf" % \
                           (ConfigSectionMap("bin_path", Config)['binbase'],
                            ConfigSectionMap("snpeff", Config)['snpeff_bin'],
                            ConfigSectionMap("snpeff", Config)['base_cmd'], raw_vcf,
                            ConfigSectionMap("bin_path", Config)['binbase'],
                            ConfigSectionMap("snpeff", Config)['snpeff_bin'],
        ...
        annotate_final_vcf_cmd = "java -Xmx4g -jar %s/%s/%s -csvStats %s_ANN.csv -dataDir %s/%s/data/ %s -c %s/snpEff.config %s %s > %s_ANN.vcf" % \
                                 (ConfigSectionMap("bin_path", Config)['binbase'],
                                  ConfigSectionMap("snpeff", Config)['snpeff_bin'],
                                  ConfigSectionMap("snpeff", Config)['base_cmd'], final_vcf,
                                  ConfigSectionMap("bin_path", Config)['binbase'],
                                  ConfigSectionMap("snpeff", Config)['snpeff_bin'],
...
def gatk_combine_variants(files_gatk, reference, out_path, merged_file_suffix, logger, Config):
    ...
    keep_logging("java -jar %s -T CombineVariants -R %s %s -o %s/Final_vcf_gatk%s" % (
    base_cmd, reference, files_gatk, out_path, merged_file_suffix),
                 "java -jar %s -T CombineVariants -R %s %s -o %s/Final_vcf_gatk%s" % (
                 base_cmd, reference, files_gatk, out_path, merged_file_suffix), logger, 'debug')
    merge_gatk_commands_file = "%s/gatk_merge.sh" % args.filter2_only_snp_vcf_dir
    with open(merge_gatk_commands_file, 'w+') as fopen:
        fopen.write("java -jar %s -T CombineVariants -R %s %s -o %s/Final_vcf_gatk%s" % (
        base_cmd, reference, files_gatk, out_path, merged_file_suffix) + '\n')
    fopen.close()
    ...
    for variants in VCF("%s/Final_vcf_gatk_no_proximate_snp.vcf.gz" % args.filter2_only_snp_vcf_dir):
        ...
        for i in gt_string_array:
            ...
            if str(code_string_array[count]) == "2" or str(code_string_array[count]) == "-2" or str(
                    code_string_array[count]) == "-3" or str(code_string_array[count]) == "-4":
...
if __name__ == '__main__':
    ...
    if "3" in args.steps:
        ...
        move_data_matrix_results = "cp -r %s/unique_positions_file %s/unique_indel_positions_file %s/*.csv %s/*.txt %s/temp_* %s/All* %s/Only* %s/*.R %s/R_scripts/generate_diagnostics_plots.R %s/" % (
        args.filter2_only_snp_vcf_dir, args.filter2_only_snp_vcf_dir, args.filter2_only_snp_vcf_dir,
        args.filter2_only_snp_vcf_dir, args.filter2_only_snp_vcf_dir, args.filter2_only_snp_vcf_dir,
        ...
        move_core_vcf_fasta_results = "mv %s/*_core.vcf.gz* %s/*_ANN* %s/*.fa %s/" % (
        ...
        if args.gubbins and args.gubbins == "yes":
            ...
            if args.outgroup:
                ...
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup),
                             ...
                             os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup),
                             logger, 'info')
                call("%s/scripts/gubbins_iqtree_raxml.sh %s 1 esnitkin_flux \'%s\'" % (
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup), logger)
                keep_logging('%s/scripts/gubbins_iqtree_raxml.sh %s 1 esnitkin_flux \'%s\'' % (
                    os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_unmapped_consensus_input, outgroup),
                             '%s/scripts/gubbins_iqtree_raxml.sh %s 1 esnitkin_flux \'%s\'' % (
                                 os.path.dirname(os.path.abspath(__file__)),
                                 prepare_ref_allele_unmapped_consensus_input, outgroup),
                             logger, 'info')
                call("%s/scripts/gubbins_iqtree_raxml.sh %s 1 esnitkin_flux \'%s\'" % (
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_unmapped_consensus_input, outgroup),
                     logger)
                ...
                keep_logging('%s/scripts/gubbins_iqtree_raxml.sh %s 1' % (
                    os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input),
                             '%s/scripts/gubbins_iqtree_raxml.sh %s 1' % (
                                 os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input),
                             logger, 'info')
                call("%s/scripts/gubbins_iqtree_raxml.sh %s 1" % (
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input), logger)
                keep_logging('%s/scripts/gubbins_iqtree_raxml.sh %s 1' % (
                    os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_unmapped_consensus_input),
                             ...
                             logger, 'info')
                call("%s/scripts/gubbins_iqtree_raxml.sh %s 1" % (
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_unmapped_consensus_input), logger)
                # call("%s/scripts/gubbins_iqtree_raxml.sh %s 1" % (os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_var_consensus_input), logger)
        else:
            if args.outgroup:
                # Get outgroup_Sample name
                ...
                keep_logging('%s/scripts/gubbins_iqtree_raxml.sh %s 0 esnitkin_flux \'%s\'' % (
                    os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup),
                             '%s/scripts/gubbins_iqtree_raxml.sh %s 0 esnitkin_flux \'%s\'' % (
                                 os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup),
                             logger, 'info')
                print "%s/scripts/gubbins_iqtree_raxml.sh %s 0 esnitkin_flux \'%s\'" % (
                os.path.dirname(os.path.abspath(__file__)), prepare_ref_var_consensus_input, outgroup)
                keep_logging('%s/scripts/gubbins_iqtree_raxml.sh %s 0 esnitkin_flux \'%s\'' % (
                    os.path.dirname(os.path.abspath(__file__)), prepare_ref_allele_unmapped_consensus_input, outgroup),
                             '%s/scripts/gubbins_iqtree_raxml.sh %s 0 esnitkin_flux \'%s\'' % (
    ...
    if "6" in args.steps:
        ...
        prepare_ref_var_consensus_input = "%s/gubbins/%s_%s_ref_var_consensus.fa" % (
        args.results_dir, (os.path.basename(os.path.normpath(args.results_dir))).replace('_core_results', ''),
        reference_base)
        prepare_var_consensus_input = "%s/gubbins/%s_%s_var_consensus.fa" % (
        args.results_dir, (os.path.basename(os.path.normpath(args.results_dir))).replace('_core_results', ''),
        reference_base)
